...
class MigrateOneCareProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migrate:one-care-products {--dry-run : Show what would be migrated without actually doing it}';
     ...
     * @var string
     */
    protected $description = 'Migrate products from one_care database to the current products table';
    ...
    {
        $this->info('Starting migration of products from one_care database...');

        // Check if one_care connection is available
        try {
            DB::connection('one_care')->getPdo();
        } catch (\Exception $e) {
            $this->error('Could not connect to one_care database. Please check your configuration.');
        ...
        $this->info("Found {$totalItems} items in one_care database.");

        $migratedCount = 0;
        $skippedCount = 0;

        $query->orderBy('id')->chunk(200, function ($oldItems) use ($oldItemsConnection, &$migratedCount, &$skippedCount) {
            $productsToInsert = [];
            ...
            foreach ($oldItems as $oldItem) {
                ...
                if (empty($productName)) {
                    $this->warn("Skipping old item ID {$oldItem->id} due to empty market_name/name.");
                    $skippedCount++;
                    continue;
                }
                ...
                if (Product::where('name', $productName)->exists() || 
                    ...
                    $this->warn("Product '{$productName}' or SKU '{$oldItem->barcode}' already exists. Skipping old item ID {$oldItem->id}.");
                    $skippedCount++;
                    continue;
                }
                ...
                if ($this->option('dry-run')) {
                    $this->line("Would migrate: {$productName} (SKU: {$productData['sku']})");
                    $migratedCount++;
                } else {
            ...
            if (!empty($productsToInsert)) {
                ...
                $migratedCount += count($productsToInsert);
                $this->info("Inserted " . count($productsToInsert) . " products.");
            }
        });

        $this->info("Migration completed!");
        $this->info("Total items processed: {$totalItems}");
        $this->info("Products migrated: {$migratedCount}");
        $this->info("Items skipped: {$skippedCount}");
    ...
    private function getOrCreateCategory($oldCategoryId, $connection)
    {
        try {
            ...
            $this->warn("Could not fetch category from old database: " . $e->getMessage());
        }
