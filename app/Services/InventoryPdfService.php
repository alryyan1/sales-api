...
class InventoryPdfService
{
    ...
    private function generateHtmlContent($products, array $filters): string
    {
        ...
        foreach ($products as $index => $product) {
            ...
            $html .= '<td>' . ($product->latest_cost_per_sellable_unit ? number_format($product->latest_cost_per_sellable_unit, 3) : '-') . '</td>';
            $html .= '<td>' . ($product->last_sale_price_per_sellable_unit ? number_format($product->last_sale_price_per_sellable_unit, 3) : '-') . '</td>';
